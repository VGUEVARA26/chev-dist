...
REM ============================================================
REM  SYNC_WEB.bat — Descarga archivos desde GITHUB (CHEV)
REM  Ejecutado remotamente via CHEV / Google Sheet
REM  Silencioso, invisible, desatendido
REM ============================================================

REM --- CONFIGURACIÓN DE GITHUB ---
set "GITHUB_USER=VGUEVARA26"
set "GITHUB_REPO=chev-dist"
set "GITHUB_BRANCH=main/CHEV"

REM --- Construcción de la URL Raw de GitHub ---
set "BASE_URL=https://raw.githubusercontent.com/!GITHUB_USER!/!GITHUB_REPO!/refs/heads/!GITHUB_BRANCH!"
set "FILES_LIST_URL=!BASE_URL!/files.txt"

REM --- Resto de la configuración local ---
...
call :LOG "============================================"
call :LOG "SYNC_WEB (GitHub) iniciado"
call :LOG "============================================"
...
REM --- Descargar files.txt desde GitHub ---
call :LOG "Descangando lista de archivos desde !FILES_LIST_URL!..."
...
if not exist "!TEMP_LIST!" (
    call :LOG "ERROR: No se pudo descargar files.txt desde GitHub."
    call :REPORT "SYNC_WEB ERROR: no se pudo conectar a GitHub"
    exit /b 1
)
...
for %%F in ("!TEMP_LIST!") do (
    if %%~zF == 0 (
        call :LOG "ERROR: files.txt está vacío."
        call :REPORT "SYNC_WEB ERROR: files.txt vacío"
        exit /b 1
    )
)
...
if !ERRORS! == 0 (
    ...
    call :LOG "Sincronización completada: !COUNT! archivos descargados."
) else (
    call :REPORT "SYNC_WEB: !COUNT! OK ^| !ERRORS! errores - !DOWNLOADED!"
    call :LOG "Sincronización con errores: !COUNT! OK, !ERRORS! fallidos."
)
...
set "MSG=%~1"
powershell -NoProfile -WindowStyle Hidden -Command ^
    "try { $msg = [uri]::EscapeDataString('!MSG!');
    Invoke-WebRequest -Uri '!APPS_SCRIPT_URL!?action=setResultado&pc_id=!PC_ID!&resultado=' + $msg -UseBasicParsing } catch {}" >nul 2>&1
goto :EOF
